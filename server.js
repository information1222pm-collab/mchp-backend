...
app.get('/api/coins', async (req, res) => {
  try {
    ...
    // ===== SOURCE 1: BIRDEYE (MULTIPLE PAGINATED CALLS - 50 MAX PER CALL) =====
    const BIRDEYE_API_KEY = process.env.BIRDEYE_API_KEY;
    if (BIRDEYE_API_KEY) {
      try {
        console.log('🐦 Fetching from Birdeye PRO (paginated, 50 per call)...');
        
        // Birdeye API limit is 50 per request, so we'll make multiple calls
        const numCalls = 10; // 10 calls × 50 = 500 tokens max
          ...
          try {
            ...
            if (birdeyeResponse.ok) {
              ...
              if (birdeyeData.success && birdeyeData.data && birdeyeData.data.tokens) {
                ...
                console.log(`✅ Birdeye batch ${i + 1}/${numCalls}: ${tokens.length} tokens`);
                
                // If we got less than 50, we've reached the end
                if (tokens.length < 50) {
                  console.log(`🏁 Birdeye: No more tokens available`);
                  break;
                }
              }
            } else {
              console.log(`❌ Birdeye batch ${i + 1} failed: HTTP ${birdeyeResponse.status}`);
            ...
            console.log(`❌ Birdeye batch ${i + 1} error:`, e.message);
          }
        }
        
        const birdeyeCount = allTokens.filter(t => t.source === 'birdeye').length;
        console.log(`✅ Birdeye TOTAL: ${birdeyeCount} tokens`);
    ...
    try {
      ...
      if (dexResponse.ok) {
        ...
        if (dexData.pairs && Array.isArray(dexData.pairs)) {
          ...
          const tokens = solanaPairs.slice(0, 100).map(p => ({
